...
seed : Seed
seed = Random.initialSeed 1441

randomList : List Float
randomList = Random.step (Random.list pointsX <| Random.float 0 1) seed
  |> Tuple.first
...
lines =
  List.repeat pointsY horizontalLine
    |> List.indexedMap (\index line ->
        let
          y = toFloat index
          dy = sin (y * pi / segmentsY)
          offset = y |> normalizeY
          transform = Mat3.translate (vec2 0 offset) Mat3.identity
