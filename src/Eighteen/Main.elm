...
vertexNormals faces positions =
  let
    normals = Array.repeat (Array.length faces) origin
  in
...
toFaces positions cells =
  let
    normals = vertexNormals cells positions
    attributes = (Array.Extra.map2 Vertex positions normals)
  in
